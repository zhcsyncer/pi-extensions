...
- [`@zhcsyncer/pi-consult`](./packages/pi-consult) — 第二意见 `consult({ why })`，带 watchdog、可选双路 panel 和本地行为日志。未配置时自行卸载。
...
- [`pi-provider-cursor-ask`](./providers/pi-provider-cursor-ask) — `@rahularya01/pi-cursor` 的整仓替代版，保留 `cursor` provider/登录身份，只映射部分模型：6 行 1M Fable/Opus/Sonnet（含 Opus 5.5）、Composer 2.5 / Fast，以及账号实时目录里有的 Grok 4.6 / Fast。
