...
- 只映射部分 Cursor 模型，不是完整目录：6 行 1M Claude（含 Opus 5.5）、Composer 2.5 / Composer 2.5 Fast，以及账号实时目录里有的 Grok 4.6 / Grok 4.6 Fast。其余系列不注册。
- Picker 使用易读名称，每个 Claude 模型只提供一行 1M，不再拆默认上下文行。
...
6 行 Claude 模型均不提供 Pi 的 thinking `off` 档。根据 Cursor 为各 Claude 模型返回的 metadata，Pi 可能提供 `low`、`medium`、`high`、`xhigh` 和 `max`；不可用的档位不会显示。Opus 5.5 在普通速度、1M 上下文下提供全部五档，需要 Cursor Max Mode。Composer 2.5 只提供 `off`（标准）和 `max`（Max Mode）。Grok 提供 `low`、`medium`、`high` 和 `xhigh`。
...
可选择 `cursor/fable-5.1`、`cursor/fable-5`、`cursor/opus-5.5`、`cursor/opus-5`、`cursor/opus-4.6`、`cursor/sonnet-5`；账号目录有时还可选 `cursor/grok-4.6` / `cursor/grok-4.6-fast`。
