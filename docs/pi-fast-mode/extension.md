...
- 需要 Pi 0.86+。内置 xAI 走 Responses。OpenAI、Codex 与内置 xAI 都通过 `registerProvider` 包官方 Responses 流；官方适配器负责序列化 `service_tier`，并按响应值调整本地费用：`priority` 通常约 2 倍，`default` 为 1 倍。token 估算只扫 transcript `messages`（含 system 字符串）。
