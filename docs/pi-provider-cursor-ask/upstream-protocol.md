...
- `tokenDelta` is progress data, not a reliable billed-output counter. When the final receipt is absent, the reply is priced as a labeled estimate instead of zeros: `output` comes from the accumulated token delta, the prompt basis is the latest context snapshot, and the cache split uses the conversation's previous-context anchor; the anchor resets on history rebuild, conversation rotation, or model switch. Estimated replies carry `billing.status: "estimated"`, never touch `totalTokens`, and emit no user-visible warning. A partial receipt prices only known buckets; cache-inclusive input is not priced as uncached when its cache split is incomplete.
...
Assistant messages carry Cursor-specific context provenance and billing-completeness metadata. Receipt-backed replies are `billing.status: "reported"`; receipt-less replies are `"estimated"` when local signal exists and `"unavailable"` when there is nothing to estimate from. Partial bills and unreported Run receipts keep a lifecycle record; the user-visible signal for those is a footer status (`ctx.ui.setStatus`), not a chat warning. Numeric zero in unknown billing buckets means **no confirmed amount is available**, not that the request was free. Estimated and receipt-backed costs are both local estimates at the configured model rates, not a reconciliation against Cursor's actual subscription debit.

Limits: context estimates are heuristic and cannot fully reconstruct unreported server-side state. Billing estimates for receipt-less Runs are context-shaped heuristics too: they cannot see server-side cache misses, they cannot tell a mid-Run tool pause from a lost final receipt, and a late real receipt may arrive after its estimate was already emitted (a small, logged overlap). Pending late receipts and the previous-context anchor are in-memory only and cannot survive process death or conversation-state eviction. Historical usage records are not rewritten.
