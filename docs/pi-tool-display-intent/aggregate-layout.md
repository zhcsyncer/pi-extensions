...
展开工具行打开统一只读快照查看器，而不是把完整原生 renderer 搬进时间线：任意扩展 renderer 可能依赖执行期状态、图片协议或交互生命周期，强行复刻会扩大兼容面。参数／元数据保留结构并脱敏，结果保留文本或 JSON；非文本附件只显示信息。窗口自身有滚动、自动换行和明确的安全截断，不调用工具、不添加消息、不影响模型上下文。

成功 Edit 的 Result 就是 diff，不另设 Diff 页；它复用本扩展已有的纯 diff 排版，而非调用工具自己的 renderer。弹窗固定单栏、增删标记和换行，避免两栏挤压阅读空间；行号和续行由 diff 排版负责，不再经过普通文本二次换行。只接受成功调用实际返回的 diff；失败优先原始错误，缺少 diff 不根据当前文件或调用参数伪造已应用的历史改动。

查看器只把 Result／Args 放在主标签中；工具额外返回的 Metadata 属于诊断层，收在 `⋯` 后，避免和主要结果抢位。Args 的键值/文本块来自同一份已脱敏的 JSON 快照，不再次访问工具对象；Raw 只是显示形式切换，不是绕过脱敏的通道。JSON 可着色，明确的自定义 Markdown 可排版，但 read/bash 等源码与日志不根据内容标点擅自解释。状态与耗时靠右，滚动位置固定在底栏右侧；只在确实变换了内容时提示脱敏/截断，不重复说明所有页面都只读。

长 steer 使用终端显示行数而非源码换行数：≤8 行完整显示，超出后保留头3尾2，中间一行提示省略数量。头尾比模型摘要更适合保留原话和末尾约束；查看需求通过同一只读窗口满足，不让 Ctrl+O 意外展开无限高日志。

## 上下文增长的口径

`ctx` 的目标是定位让上下文膨胀的步骤，不是展示又一份当前水位或计费账。默认关闭，用一个开关控制收据总计和 `turns` 拍头；不在工具行分摊，因为父请求没有逐工具结果的真实 token 计数。启用时，拍编号覆盖最终文本与仅透传工具的拍，它们用框外轻量尾注承载；没有工具 leader 时不制造空 Tools 框。
