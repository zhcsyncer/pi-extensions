...
- [`@zhcsyncer/pi-herdr-companion`](./packages/pi-herdr-companion) — standalone Herdr companion with stable runtime context, owned process panes, durable `/btw` side threads, and blocked-state adapters. It is intentionally not auto-enabled by the root bundle.
