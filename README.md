...
- [`@zhcsyncer/pi-tool-display-intent`](./packages/pi-tool-display-intent) — compact tool rendering with model-written intent phrases, RPC-visible summaries, an optional theme-colored aggregate Activity with stable `done` rows, adaptive diffs, and bounded Bash call previews.
