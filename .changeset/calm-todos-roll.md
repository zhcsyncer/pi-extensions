...
Tighten default model guidance so risk, duration, or importance cannot justify a singleton Todo cycle: fresh plans start with an atomic multi-item batch in intended execution order, distinct outcomes stay split, and one-milestone work runs directly without filler tasks.
