...
显式目标可以是 `--local`、`--base <ref>` 或 `--range <refA>..<refB>`；它们会覆盖智能推断。`--local` 是明确的本地/离线模式，不 fetch；`--base` 和 `--range` 引用 remote-tracking ref 时会先刷新对应 remote。可选参数包括 `--reqdoc <path>`、`--focus <text>`、`--gating weighted|strict`，以及显式确认大目标的 `--allow-large`。没有显式模型 scope 时命令会直接拒绝。TUI 运行期间，footer 显示整轮聚合进度，按 Esc 会通过同一 stop 路径取消整个 fleet。只有选中兼容的外部 Subagents runtime 时才提供每路 FleetView；embedded 运行的同等 route 细节仍完整保留在 audit report。

Whole-target 的建议阈值是 200 KiB 或 5,000 个逻辑行。这是评审质量与资源信号，不是 Git 限制：同一份 patch 和附加上下文会复制给 2–8 个隔离 reviewer；大目标会稀释证据，也会成倍增加上下文占用、延迟、内存和 provider 成本。与依赖各家 tokenizer 的估算相比，字节数和行数还能在不同 provider 间保持确定性。超过建议阈值时，TUI 会询问是整体评审还是查看较小范围；headless 必须显式传 `--allow-large`。获准整体运行的大目标把 reviewer turns 从 25 提高到 40、refuter turns 从 12 提高到 20，同时保留可配置的 grace turns（默认 5）和 wall-clock timeout 防止失控（reviewer：单路 10 分钟 / 整轮 20 分钟；refuter：5 / 15 分钟）。绝对 fail-loud 上限是 1 MiB 或 25,000 行。
