...
- 最终报告是另一条独立的持久 transcript 节点。折叠态即可开始裁决：结论、短 target、blocking 标题、失败 route、不完整的 Refute；展开才补全 finding、每路终态，以及已持久化的 session 路径。TUI 成功路径不再另外 toast target 或完成摘要。
