...
Each unremembered scoped model starts as `disabled`. Its first enabled value is `medium`, or Pi AI's nearest supported level when that model does not expose `medium`; later cycling still exposes every supported level. A scope-pinned model can only use `disabled` or the pinned level. The same setup picker includes **Refute blocking findings**, defaulting to a fresh session with the current main-session model and exact thinking level. It can be changed to **choose model** (opening the scoped single-route picker after reviewer confirmation) or `disabled`; if the main-session route is unavailable or incompatible, setup falls back to **choose model** instead of blocking review. Confirm 2–8 routes with **Run selected reviewers**; Escape cancels before any reviewer starts. Valid reviewer/refuter model choices are remembered only for the current Pi session and removed scope entries are not restored.

The ordinary no-flag TUI setup enables Refute by default with the current main-session model/thinking route, even when that model is outside the reviewer scope. Choosing another model opens the independent single-route picker. Explicit reviewer flags bypass that setup control, so pass `--refute` to enable the same main-session default in TUI, or combine it with an exact `--refuter` override. Non-interactive modes remain reproducible and require both `--refute` and an exact scoped `--refuter`:
