...
import type { RangeCheckoutEstimate } from "./git-target.ts";
import {
  assertRangeCheckoutCapacity,
  createHardenedWorktreeEnv,
  createRangeCheckoutMonitor,
  runWorktreeGit,
  type RangeCheckoutDependencies,
} from "./range-checkout.ts";

export {
  assertRangeCheckoutCapacity,
  MAX_RANGE_CHECKOUT_ENTRIES,
  MAX_RANGE_CHECKOUT_LOGICAL_BYTES,
  MIN_RANGE_CHECKOUT_FREE_BYTES,
} from "./range-checkout.ts";
export type {
  RangeCheckoutDependencies,
  RangeCheckoutPolicy,
} from "./range-checkout.ts";
...
export async function createReviewTempWorkspace(
  ...
  return {
    ...
    async createRangeWorktree(createOptions) {
      ...
      const env = await createHardenedWorktreeEnv(
        createOptions.root,
        hooksDir,
        createOptions.signal,
      );
      const commonGitDir = path.resolve(createOptions.root, await runWorktreeGit({
      ...
      await runWorktreeGit({
        ...
        monitor: createRangeCheckoutMonitor(runDir, createOptions.dependencies),
      });
      return worktreeDir;
    },
    async cleanup() {
      ...
      if (ownership.state === "pending" && registration) {
        ...
        if (checkoutMissing) {
          ...
          ownership = removing;
          const cleanupEnv = await createHardenedWorktreeEnv(worktreeDir, hooksDir);
          try {
            await runWorktreeGit({
              root: worktreeDir,
