...
4. **终态 transcript entry：**持久的 `adversarial-review-result`、cancellation 或 error entry 关闭可见生命周期。折叠报告即可开始裁决：结论、短 target、非零计数、最多三条 blocking 标题、有界的 route failure、不完整或失败的 Refute，以及 stale/cancelled/inconclusive/failed 警告。展开补全 finding、contested 理由和每路终态，不含运行时账本（waves、timeout、并发、token、print 模式注释）；有持久化 child-session 路径时会显示。Adjudication handoff 使用另一条隐藏 custom message，因此不会重复显示最终报告。TUI 成功路径不再 toast preflight target 或完成摘要。
