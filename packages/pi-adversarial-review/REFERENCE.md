...
4. **Terminal transcript entry:** the durable `adversarial-review-result`, cancellation, or error entry closes the visible lifecycle. A collapsed report is adjudication-ready: verdict, short target, non-zero counts, up to three blocking titles, bounded route failures, and a warning when the run is stale, cancelled, inconclusive, or failed. Expansion adds complete findings, contested reasons, and per-route outcomes without the runtime ledger (waves, timeouts, concurrency, token totals, print-mode notes). The adjudication handoff is a separate hidden custom message, so the visible report is not duplicated. Successful TUI runs do not toast the preflight target or completion summary.
