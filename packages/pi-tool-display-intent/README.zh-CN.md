...
展开时会保留聚合标题在视口中的位置，不再追随新增内容跳到末尾。向下阅读较长的展开账本、标题滚出视口上方后，输入框上方会出现固定的 **`↑ Run (…) · Collapse`** 收起条。点击只收起当前账本，并回到它的聚合标题；不会抢输入焦点或覆盖正文。这些视口功能需要兼容的 Pi 0.85+ fullscreen 渲染器，普通终端滚动模式不提供账本鼠标交互。
