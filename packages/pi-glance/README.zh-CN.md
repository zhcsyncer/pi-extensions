...
- **Context**：百分比、当前 token / context window、右下角进度条，以及压缩后的未知状态。
...
- **Context progress**：在 `/glance` → **Context** → `Display` 中选择 `progress bar` 后，context 会离开顶部状态并显示为与边框同基线的自适应 track，例如 `╶───────────╴ 23%`。已用部分使用 context 色，剩余部分使用 dim 色，百分比使用普通文本色。底部进度条有意省略 context 图标，使其与圆角边框保持一致；`nerd` 文本模式仍使用 `󰍛` memory 图标。
- **Auto compact**：Pi 自动压缩开启时显示。`plain` 模式高亮 `auto`，`nerd` 模式高亮 `󰁄 auto`；可在 **Bottom details** 中单独隐藏。该状态反映 Pi 合并后的全局/项目设置，项目设置仅在项目受信任时读取。

两项同时显示时，context progress 与 auto compact 会自适应占用输入框内部宽度的三分之一。窄终端会优先缩短进度条；极窄终端中 context 优先于 auto compact。对应的唯一右下角配置为：
...
配置保存在 `~/.pi/agent/pi-glance/config.json`。当前 schema 为版本 9；旧配置会自动迁移并丢弃已废弃的 Footer 和详情开关。
