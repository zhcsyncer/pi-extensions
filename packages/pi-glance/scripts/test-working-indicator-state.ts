...
state.messageUpdate("thinking_delta", 1_100);
state.setPartialEstimate(42);
...
assert.equal(state.snapshot.hasPartialEstimate, true, "positive partial estimates should be visibly marked");

state.messageUpdate("toolcall_delta", 1_200);
state.setPartialEstimate(55);
...
state.messageUpdate("text_delta", 2_100);
state.setPartialEstimate(7);
...
state.messageUpdate("text_delta", 3_000);
state.setPartialEstimate(3);
...
state.messageUpdate("text_delta", 13_001);
assert.equal(isWorkingStalled(state.snapshot, 13_001), false, "new assistant progress should immediately clear stall before the next estimate tick");
state.turnStart(13_002);
state.messageUpdate("text_start", 13_003);
state.setPartialEstimate(0);
assert.equal(state.snapshot.hasPartialEstimate, false, "an empty partial should not expose a meaningless estimate marker");
...
state.messageUpdate("text_delta", 100_000);
state.setPartialEstimate(5);
