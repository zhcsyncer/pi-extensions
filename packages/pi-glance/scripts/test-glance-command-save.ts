...
function createContext(customResults: PaneResult[]): TestContext {
	...
	const ctx = {
		...
		ui: {
			...
			setWidget: (_key: string, factory: unknown) => surfaceCalls.push(factory ? "setWidget:install" : "setWidget:clear"),
...
async function main(): Promise<void> {
	...
	try {
		...
		assert.deepEqual(
			...
			["setWidget:clear", "setEditorComponent:clear", "setFooter:clear"],
			"successful save of disabled config should clear only the owned Widget/editor/footer surface after disk write succeeds",
		);
