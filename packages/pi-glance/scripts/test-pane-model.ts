...
assert.equal(settingsWrapUp.model.settingIndex, 6, "up from first Git setting should wrap to the last Git setting");
assert.equal(selectedSetting(view(settingsWrapUp.model)).id, "git.polling", "up in settings should wrap to Git polling");

const gitValuesBottom = withFocus(model, "values", 2, 6);
