...
async function main(): Promise<void> {
	...
	try {
		...
		assert.equal(JSON.parse(await readFile(configPath, "utf8")).version, 14, "migration should write the current schema version");
		...
		assert.equal(JSON.parse(await readFile(configPath, "utf8")).git.worktreeSummary, "above-compact", "schema migration should persist the default working tree summary mode");
