...
`Ctrl+Alt+S` stashes or restores the current editor draft. `Ctrl+Alt+U` discards the stash. The left border shows a short mark while a draft is waiting. Reloading or resuming the same session restores it automatically if the editor is empty.
...
| 📥 | **Editor stash** | `Ctrl+Alt+S` stashes or restores, `Ctrl+Alt+U` discards; the border marks an unrestored draft |
