...
`Ctrl+Shift+S` stashes or restores the current editor draft. `Ctrl+Shift+U` discards the stash after a second press. Confirm prompts appear under the editor with a short draft preview and disappear if you wait. The left border shows a short mark while a draft is waiting. Reloading or resuming the same session restores it automatically if the editor is empty.
...
| 📥 | **Editor stash** | `Ctrl+Shift+S` stashes or restores, `Ctrl+Shift+U` discards after a second press; the border marks an unrestored draft |
