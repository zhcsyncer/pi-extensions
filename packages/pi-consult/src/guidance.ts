export const DEFAULT_PROMPT_SNIPPET =
	"Ask consult({ why }) for a second opinion before structural decisions, after repeated failure, or before declaring done";

export const DEFAULT_PROMPT_GUIDELINES: string[] = [
	"Call `consult` with a required `why` (1-2 sentences) before choosing among ≥2 approaches that will shape later structure.",
	...
	"Call `consult` before you claim the task is complete, after any substantive edit/write work.",
	...
	"After each `consult` result, the next visible reply must restate the summary and declare adopt or reject with a reason as a `CONSULT-LOG:` line. Users often cannot see collapsed tool results.",
];
