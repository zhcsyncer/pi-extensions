...
describe("consult tracker gates", () => {
	it("keeps the gate locked until consult ends, then requires a fresh loop", () => {
		const tracker = new ConsultTracker();
		for (let i = 0; i < 3; i++) {
			tracker.onToolStart(String(i), "bash", { command: `probe-${i}` });
			tracker.onToolEnd(String(i), "bash", true);
		}
		expect(tracker.evaluateLoop(3, budget)).toEqual({ fire: true, reason: "error" });
	...
	it("an explicit pull consult also clears earlier loop evidence", () => {
		const tracker = new ConsultTracker();
		tracker.onToolStart("old", "bash", { command: "old-probe" });
		tracker.onToolEnd("old", "bash", true);
		expect(tracker.consumeTrigger()).toBe("pull");
		...
		expect(tracker.evaluateLoop(2, budget).fire).toBe(false);
	});
