...
- When every current task is completed or deleted, begin the next cycle with the required multi-create batch; rollover occurs automatically before that batch. Previous-cycle tasks leave live state and are available only through the transcript/tree. For API and replay compatibility, the reducer still accepts legacy top-level-create rollover, but default model guidance does not use it.
