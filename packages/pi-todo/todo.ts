...
export function registerTodoTool(pi: ExtensionAPI, store: TodoStore, config: TodoConfig = loadConfig()): void {
	...
	pi.registerTool({
		...
		label: TOOL_LABEL,
		description:
			"Manage a multi-item execution plan for meaningful multi-stage work. Actions: create, update, list, get, delete, and batch. Never start a one-task Todo cycle, regardless of risk, duration, or importance. Start a fresh cycle with an atomic batch containing at least two independently valuable create operations; use top-level create only to add a newly discovered milestone to an active plan. Batch operations run in order and roll back atomically. When the current cycle is terminal, such a batch rolls it over automatically.",
