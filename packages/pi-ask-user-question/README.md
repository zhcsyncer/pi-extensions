...
该 fork 将问卷放入 Pi 的正常自定义组件布局，而不是覆盖底部区域的全屏 overlay；同时支持数字键直选，并为 tool call/result 提供可审计渲染。
...
每道题会自动追加 `Type something.`，用于输入选项之外的回答。数字快捷键只对应模型提供的正式选项，不会误触该输入行或 `Next`；在自定义回答和备注编辑器中，数字仍作为普通文本输入。

## TUI 布局与工具展示

问卷通过非 overlay 的 `ctx.ui.custom()` 渲染：交互期间临时替换底部 editor、保留并在结束后恢复 editor 草稿，同时参与 Pi 正常布局计算，使 transcript 重排且 footer 始终单独可见。

工具调用默认展示全部问题、选项标签和说明；带 preview 的选项会明确标记。展开工具节点后可阅读受长度限制的 preview。结果节点展示每道题的答案、回答类型和备注；取消时仍保留可审计的部分回答，校验与运行错误始终可见。
