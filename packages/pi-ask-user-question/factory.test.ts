...
describe("ask_user_question — single-question navigation", () => {
	it("number key selects the corresponding option without navigation", async () => {
		const tool = register();
		const { custom } = driveCustom((c) => c.handleInput("2"));
		const ctx = { hasUI: true, ui: { custom } } as never;
		const r = (await tool.execute?.("tc", threeOptionParams as never, undefined as never, undefined as never, ctx)) as
			| ToolResult
			| undefined;
		expect(r?.details).toMatchObject({
			cancelled: false,
			answers: [{ questionIndex: 0, answer: "Beta", kind: "option" }],
		});
	});
...
describe("ask_user_question — multi-select flow (single question)", () => {
	...
	it("number key toggles the corresponding checkbox directly", async () => {
		const tool = register();
		const { custom } = driveCustom((c) => {
			c.handleInput("2"); // Backend ON; focus remains on Frontend
