...
- Number keys `1`–`N` directly choose or toggle authored options, while `N+1` focuses `Type something.`; subsequent digits become text and the `Next` sentinel remains navigation-only.
